#!/bin/bash
# PXD GitHub one-click installer (full). Downloads pxd-full-father.tar.gz from Releases.
set -o pipefail
...
for arg in "$@"; do
    case "$arg" in -h|--help)
        echo 'PXD full installer: bash install [install.sh options]'
...
if [ "$RELEASE_TAG" = latest ]; then
    ...
    PKG_URL=${PXD_PACKAGE_URL:-https://github.com/${REPO}/releases/download/${RELEASE_TAG}/pxd-full-father.tar.gz}
fi
...
if [ -f "$TEMP_DIR/package/install.sh" ]; then
    ...
    cd "$TEMP_DIR/package/pxd_full_father" || exit 1
fi
